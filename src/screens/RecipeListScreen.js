...
const RecipeListScreen = ({route }) => {
  ...
  const searchResults = route.params?.searchResults;
	...
	const fetchData = async () => {
		...
		try {
			if (searchResults) {
				...
				const response = await apiClient.post('/recipes/find-by-ingredients', {
					myIngredients: searchResults,
				});
				setAllRecipes(response.data);
			} else {
				// Otherwise, fetch all recipes and categories as normal
	...
	}, [isFocused, searchResults]); // Rerun if searchResults change
